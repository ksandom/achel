		...
		foreach ($tagsArray as $tag)
		{
			if (isset($this->store['Tags'][$tag]))
			{
				foreach ($this->store['Tags'][$tag] as $name)
				{
					...
					{
						$this->debug($this->l0, "searchHelp: Could not find a feature with the name \"$name\". This can happen if the primary flag for a feature is not in the list of flags for that feature.");
					}
				}
			}
