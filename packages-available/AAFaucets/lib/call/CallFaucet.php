	...
	function preGet()
	{
		...
		foreach ($this->input as $channel=>$data)
		{
			if ($data)
			{
				if ($this->semiInline)
				{
					foreach ($data as $line)
					{
						if (!is_object($line) and !is_array($line))
						{
							...
							$this->debug($this->l3, "CallFaucet->preGet: Calling semiInline feature={$this->feature} parameter=$parameter");
				...
				{
					$this->debug($this->l3, "CallFaucet->preGet: Calling feature={$this->feature} parameter={$this->argument}");
