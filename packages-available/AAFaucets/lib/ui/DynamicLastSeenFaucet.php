...
class DynamicLastSeenFaucet extends ThroughBasedFaucet
{
	...
	function control($feature, $value)
	{
		...
		{
			case 'forgetChannel':
				$this->debug($this->l2, __CLASS__.'->'.__FUNCTION__.": Issueing forget for channel $value.");
				$this->forgetChannel($value);
				break;
			case 'forgetOlderThan':
				$this->debug($this->l2, __CLASS__.'->'.__FUNCTION__.": Issueing forget for channels older than $value seconds.");
				$this->forgetOlderThan($value);
				break;
			case 'forgetAll':
				$this->debug($this->l2, __CLASS__.'->'.__FUNCTION__.": Issueing forget for all channels.");
				$this->forgetAll();
				break;
