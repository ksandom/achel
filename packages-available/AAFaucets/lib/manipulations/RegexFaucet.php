		...
		foreach ($this->input as $channel=>$data)
		{
			if ($data)
			{
				if (count($rules))
				{
					...
					{
						$this->debug($this->l3, "RegexFaucet->preGet: Delivering line");
						$delivered=false;
				...
				{
					$lines=count($data);
					$this->debug($this->l3, "RegexFaucet->preGet: no rules set, so sending $lines lines of ".gettype($data)." output to $defaultOut");
