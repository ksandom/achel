		...
		*/
		...
		if (is_array($data))
		{
			foreach ($data as $line)
			{
				...
				elseif (!is_bool($line)) $this->debug($this->l2, __CLASS__.'->'.__FUNCTION__.": Expected an array of strings, but struck ".gettype($line)." in the array.");
			}
		}
		elseif(!is_bool($data)) $this->debug($this->l2, __CLASS__.'->'.__FUNCTION__.": Expected an array of strings, but got ".gettype($data));
	}
}
