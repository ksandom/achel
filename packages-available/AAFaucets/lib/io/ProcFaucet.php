...
class ProcFaucet extends Faucet
{
	...
	function control($feature, $value)
	{
		switch ($feature)
		{
			...
			case 'code':
				# TODO put in semantics for this. It should be defined when the object is created.
				$this->debug($this->l2, "Sending control $value to {$this->cmd}");
				$code="\033$value";
			...
			case 'localKill':
				...
				print_r($status);
				if (posix_kill($status['pid'], $signal)) $this->debug($this->l2, "Success sent signal $signal to {$this->cmd}");
				else $this->debug($this->l2, "Failure sending signal $signal to {$this->cmd}");
				break;
			default:
			...
			switch ($channel)
			{
				...
				default:
					foreach ($data as $line)
					{
						...
						else $this->debug($this->l1, __CLASS__.'->'.__FUNCTION__.": Expected an array of strings, but struck ".gettype($line)." in the array.");
					}
					break;
			}
		}
