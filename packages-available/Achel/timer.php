...
class Timer extends Module
{
	...
	function createTimer($name, $delay=100000)
	{
		...
		$this->core->debug(2, "createTimer: Adding timer $name");
	...
	{
		$timers=$this->core->get('Timer', 'timers');
		$this->core->debug(2, "deleteTimer: Deleting timer $name");
		unset($timers[$name]);
	...
	function loadTimers()
	{
		$this->core->debug(3, "loadTimers: Loaded/reloaded timers");
		$this->timers=&$this->core->get('Timer', 'timers');
		
		foreach ($this->timers as $name=>$delay)
		{
			$this->core->debug(3, "loadTimers:   $name: $delay");
		}
		
		$this->core->debug(3, "loadTimers: Mainloop timer intervals");
		...
		{
			$this->core->debug(3, "loadTimers:   $pos: $interval");
		}
	}
	...
	function mainLoop()
	{
		...
		{
			$this->core->debug(1, "mainLoop: No timers created. If the mainLoop was allowed to start, it would be an uncontrollable infinite loop with no purpose.");
			return false;
		}
		
		if ($this->mainLoopRunning)
		{
			$this->core->debug(1, "mainLoop: The main loop has already been started. Allowing it to be started a second time could do baAaAaAad things!");
			return false;
		}
		...
		$this->core->debug(2, "mainLoop: Entering the loop.");
	...
	function stopMainLoop()
	{
		...
		$this->core->debug(2, "stopMainLoop: Issued stop.");
	}
}
