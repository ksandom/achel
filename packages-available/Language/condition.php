...
class Condition extends Module
{
	...
	function event($event)
	{
		switch ($event)
		{
			...
			case 'else':
				...
				break;
			case 'ifNested':
				if ($featureName=$this->ifNested(explode(valueSeparator, $this->core->get('Global', $event))))
				{
					return $this->core->callFeature($featureName);
				}
				break;
			default:
	...
	function ifNotEmptyResult($input, $parms, $match=true)
	{
		...
		*/
		if ($matched == $match)
		{
			$this->debug($this->l4, "ifNotEmptyResult: got here D4");
			$this->core->incrementNesting();
			$result=$this->takeAction($parms);
			$this->core->decrementNesting();
			$this->debug($this->l4, "ifNotEmptyResult: got here D5");
		}
		...
		$this->debug($this->l5, "ifNotEmptyResult: Just about to return. Count=".$this->core->getResultSetCount());
		return $result;
	}
	...
	function doIf($value1, $comparison, $value2)
	{
		...
		switch ($comparison)
		{
			...
			case '<=':
				...
				break;
			default:
				$this->debug($this->l0, "Condition: Unknown comparison \"$comparison\" in \"$value1,$comparison,$value2\"");
				$result=false;
				break;
		}
	...
	function ifNested($parts)
	{
		$lastPos=count($parts)-1;
		$keys=array_keys($parts);
		$lastKey=$keys[$lastPos];
		$lastValue=$parts[$lastKey];
