...
class BalanceFrustration extends BalanceVectorAlgorithm
{
	...
	function process($ruleName, &$rule)
	{
		...
		{ // We will overshoot
			...
			{ // not overshot
				$this->debug($this->l2, "v  pan {$rule['input']['live']['vector']['distanceRemaining']} -- {$rule['output']['live']['value']} will overshoot ({$rule['input']['overshootTime']}>{$rule['input']['live']['vector']['timeRemaining']}) and {$rule['input']['live']['vector']['distanceRemaining']}>0");
				#$this->decelerate($rule);
				$this->panicDecelerate($rule);
			}
			else
			{ // overshot
				$this->debug($this->l2, " ^ res {$rule['input']['live']['vector']['distanceRemaining']} -- {$rule['output']['live']['value']} have overshot ({$rule['input']['overshootTime']}>{$rule['input']['live']['vector']['timeRemaining']}) and {$rule['input']['live']['vector']['distanceRemaining']}!>0");
		...
		else
		{
			...
			{ // We are below max speed
				$this->debug($this->l2, " > acc {$rule['input']['live']['vector']['distanceRemaining']} -- {$rule['output']['live']['value']} speed ({$rule['input']['live']['vector']['speed']}<{$rule['input']['maxChangePerSecond']})");
				$this->accelerate($rule);
			}
			else
			{ // We have crossed max speed
				$this->debug($this->l2, "<  dec {$rule['input']['live']['vector']['distanceRemaining']} -- {$rule['output']['live']['value']} speed ({$rule['input']['live']['vector']['speed']}!<{$rule['input']['maxChangePerSecond']})");
				$this->decelerate($rule);
			}
		}
