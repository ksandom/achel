...
class BalanceDirect extends BalanceAlgorithm
{
	...
	function economyTests($ruleName, &$rule)
	{
		...
		if ($rule['input']['live']['inputGoal']==$rule['input']['center'])
		{ // Input is at center.
			$this->debug($this->l3, __CLASS__.'->'.__FUNCTION__.": $ruleName: branch: center {$rule['input']['live']['inputGoal']}=={$rule['input']['center']}");
			$output=$rule['output']['center'];
			$branch='center';
		}
		elseif ($rule['input']['live']['inputGoal']<=$rule['input']['min'])
		{ // Input is at or below lower boundary.
			$this->debug($this->l3, __CLASS__.'->'.__FUNCTION__.": $ruleName: branch: lower boundary {$rule['input']['live']['inputGoal']}<={$rule['input']['min']}");
		...
		{ // Input is at or above upper boundary.
			$this->debug($this->l3, __CLASS__.'->'.__FUNCTION__.": $ruleName: branch: upper boundary {$rule['input']['live']['inputGoal']}>={$rule['input']['max']}");
		...
		{ // Input is between center and lower boundary.
			$this->debug($this->l3, __CLASS__.'->'.__FUNCTION__.": $ruleName: branch: lower {$rule['input']['live']['inputGoal']}<{$rule['input']['center']}");
			$output=$this->getBetween($rule['input']['live']['inputGoal'], $rule, 'min', 'center');
			$branch='lower';
		}
		elseif ($rule['input']['live']['inputGoal']>$rule['input']['center'])
		{ // Input is between center and upper boundary.
			$this->debug($this->l3, __CLASS__.'->'.__FUNCTION__.": $ruleName: branch: upper {$rule['input']['live']['inputGoal']}>{$rule['input']['center']}");
